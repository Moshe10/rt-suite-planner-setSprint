...
const reducer = function (initState, action) {
    ...
    switch (action.type) {
        ...
        case "SELECT_DEV_TO_CONT":
            ...
            if (myNewContainer.developers.filter((dev) => (dev.name !== action.payload.developer.name)).length !== myNewContainer.developers.length) {
                myNewContainer.developers = myNewContainer.developers.filter((dev) => (dev.name !== action.payload.developer.name));
            } else {
                myNewContainer.developers = [...myNewContainer.developers, action.payload.developer];
            }
